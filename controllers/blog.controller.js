...
const createBlog = async (req, res) => {
  ...
  // req.file will contain the uploaded file from the form
  ...
  try {
    const serviceResponse = await blogService.createBlog({
      ...
      state,
      tags,
      image_url, // Pass the file data (req.file) to the service
    });
    ...
    res.status(serviceResponse.code).render("create", {
      serviceResponse,
    });
...
const getAllPublishedBlogs = async (req, res) => {
  ...
  if (author && author.trim()) {
    ...
    if (authorNames[0])
      nameQuery.first_name = { $regex: new RegExp(authorNames[0], "i") };
    if (authorNames[1])
      nameQuery.last_name = { $regex: new RegExp(authorNames[1], "i") };
  ...
  try {
    const serviceResponse = await blogService.getAllPublishedBlogs({
      filter,
      page: currentPage,
      perPage: limit,
      sortBy,
    ...
    res
      .status(500)
      .json({
        message: err.message || "Unable to retrieve all post",
        error: process.env.NODE_ENV === "development" ? err.stack : undefined,
      });
  }
};
...
const getThisAuthorBlogs = async (req, res) => {
  ...
  try {
    const serviceResponse = await blogService.getThisAuthorBlogs(
      user_id,
      loggedInUser,
      state,
      page,
    ...
    res
      ...
      .render("dashboard", {
        blog: serviceResponse.data.blogs,
        user: req.user,
        authorId: user_id,
      });
  } catch (err) {
...
const getBlogById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(serviceResponse.code)
      .render("singleblog", {
        blog: serviceResponse.data.blog,
        user: res.locals.user,
      });
  } catch (err) {
    console.log(err);
    res
      .status(500)
      .json({ message: err.message || "Unable to retrieve this post" });
  }
};
...
const getUpdateBlogId = async (req, res) => {
  ...
  try {
    ...
    res
      .status(serviceResponse.code)
      .render("updateblog", { blog: serviceResponse.data.blog });
  } catch (err) {
    console.log(err);
    res.status(500).json({
...
const updateBlog = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: err.message || "Unable to update this post" });
  }
};
